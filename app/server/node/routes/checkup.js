...
// 上传临时目录：放到持久化数据目录，避免写入应用安装目录（升级会丢 / 可能只读）
const UPLOAD_TMP_DIR = path.join(config.DATA_DIR, 'uploads');
try { if (!fs.existsSync(UPLOAD_TMP_DIR)) fs.mkdirSync(UPLOAD_TMP_DIR, { recursive: true }); } catch (e) { /* 运行时再建 */ }

// 手机端常见：mimetype 为空或 application/octet-stream，此时按扩展名兜底判断，避免误拒
function _isAllowedUpload(file, allowedMimes, allowedExts) {
  const mime = ((file && file.mimetype) || '').toLowerCase();
  if (allowedMimes.includes(mime)) return true;
  if (!mime || mime === 'application/octet-stream' || mime === 'image/jpg' || mime === 'image/heif') {
...
// 包一层：把 multer 的错误（文件过大/上传异常）转成 JSON，避免前端只看到「服务器内部错误」
function uploadSingle(field) {
  const mw = upload.single(field);
  return (req, res, next) => mw(req, res, (err) => {
    if (err) {
      const msg = err.code === 'LIMIT_FILE_SIZE' ? '文件过大，最大支持 50MB' : (err.message || '文件上传失败');
      logger.warn('checkup', `上传失败: ${msg}`);
...
function _nasRoots() {
  const set = new Set();
  const addAll = (val) => {
    for (const p of String(val || '').split(':').map(s => s.trim()).filter(Boolean)) {
      set.add(p.replace(/\/+$/, '') || '/');
    }
  };
  addAll(process.env.TRIM_DATA_SHARE_PATHS);
  addAll(process.env.TRIM_DATA_ACCESSIBLE_PATHS);
  for (let i = 1; i <= 10; i++) set.add('/vol' + i);
  set.add('/home');
  set.add('/share');
  return [...set];
}
const NAS_WHITELIST = _nasRoots();
...
function _isNasPathSafe(p) {
  ...
  if (normalized === '/') return true; // 虚拟根：允许从根开始浏览
  return _nasRoots().some(allowed => normalized === allowed || normalized.startsWith(allowed + '/'));
}
...
router.post('/checkups/:id/photos', uploadSingle('file'), async (req, res) => {
  try {
    ...
    if (!_isAllowedUpload(req.file, ALLOWED_PHOTO_MIMES, PHOTO_EXTS)) {
      try { fs.unlinkSync(req.file.path); } catch (e) {}
      return res.json({ code: 1001, data: null, message: '不支持的文件类型，仅允许 JPG/PNG/WebP/GIF/HEIC/PDF' });
    }
...
router.post('/checkups/:id/reports', uploadSingle('file'), async (req, res) => {
  try {
    ...
    if (!req.file) return res.json({ code: 1001, data: null, message: '请上传文件' });
    logger.info('checkup', `POST /checkups/${req.params.id}/reports - file=${req.file.originalname}, mime=${req.file.mimetype}, size=${req.file.size}`);
    if (!_isAllowedUpload(req.file, ALLOWED_REPORT_MIMES, REPORT_EXTS)) {
      try { fs.unlinkSync(req.file.path); } catch (e) {}
      return res.json({ code: 1001, data: null, message: `不支持的文件类型(${req.file.mimetype || '未知'})，仅支持图片(JPEG/PNG/WebP/HEIC)和PDF` });
    }
...
router.get('/files/browse', async (req, res) => {
  try {
    ...
    // 根目录：作为虚拟入口，列出所有实际存在的可访问卷，避免“从根开始浏览”被白名单直接拒绝
    if (normalized === '/' || normalized === '.') {
      const entries = _nasRoots()
        .filter(p => { try { return fs.existsSync(p) && fs.statSync(p).isDirectory(); } catch { return false; } })
        .map(p => ({
      ...
      return res.json({ code: 0, data: { path: '/', parent: null, entries }, message: 'success' });
    }
    ...
    // 父目录：卷根（如 /vol1）的上一层回到虚拟根 '/'
    const absParent = path.dirname(normalized).replace(/\\/g, '/');
    const parentPath = _isNasPathSafe(absParent) ? absParent : '/';
