...
// 上传临时目录：放到持久化数据目录（默认 process.cwd()/data 会落到应用安装目录，升级会丢/可能只读）
...
// 把 multer 的错误（体积/异常）转成 JSON，避免前端只看到「服务器内部错误」
function uploadSingle(field) {
  const mw = upload.single(field);
  return (req, res, next) => mw(req, res, (err) => {
    if (err) {
      const msg = err.code === 'LIMIT_FILE_SIZE' ? '文件过大，最大支持 100MB' : (err.message || '文件上传失败');
      logger.warn('photo', `上传失败: ${msg}`);
...
router.post('/photos', uploadSingle('file'), async (req, res) => {
  try {
