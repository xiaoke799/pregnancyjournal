 ...
 */
// 静态产检时间表（id → {name, week_range}）。schedule_dates.schedule_id 指向的就是这张表的 id，
// 该表不在数据库里（是内置数据），此前错误地 JOIN 了 prenatal_checkup 的 name/week_range 列，
// 导致推送报「no such column / no such table」。
let _scheduleMapCache = null;
function getScheduleMap() {
  if (_scheduleMapCache) return _scheduleMapCache;
  const map = {};
  try {
...
async function executeDailyPush(pregnancyId, config, sourceType, existingLogId) {
  ...
  try {
    ...
    for (const c of tomorrowCheckups) lines.push(`⚠️ 【产检】${c.name || c.schedule_id} ${c.week_range || ''}`);
    ...
    for (const c of upcomingCheckups) {
      ...
      lines.push(`🩺 ${c.name || c.schedule_id} ${c.week_range || ''} - ${c.checkup_date} ${label}`);
    }
