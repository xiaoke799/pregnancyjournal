...
function _flush() {
  // ⚠️ 本函数由 setInterval 调用：同步抛出会变成 uncaughtException → process.exit(1)。
  // JSON.stringify 在 meta 含循环引用时会抛，故整体兜底（日志绝不能反噬业务）。
  try {
    ...
    for (const entry of batch) {
      ...
      try {
        ...
        line = JSON.stringify({ ts: entry && entry.ts, level: entry && entry.level, category: entry && entry.category, message: '[meta 无法序列化]' }) + '\n';
      }
      appLines.push(line);
    ...
    _writing = false;
    try { console.error('[logger] flush 异常:', e && e.message); } catch (_) { /* ignore */ }
  }
}
...
function log(level, category, message, meta) {
  // 日志绝不能反噬业务：任何异常一律吞掉。
  // 历史事故：log.api 方法缺失，在 PDF 导出流的 doc.on('end') 回调里抛出，
  // 触发 uncaughtException → process.exit(1)，导致整个应用白板。
  try {
  ...
  if (CONFIG.enableFile && _ready) {
    ...
    try { console.error('[logger] 写日志失败:', e && e.message); } catch (_) { /* 彻底静默 */ }
  }
}
...
const logger = {
  ...
  // API/业务操作日志（导出、备份、恢复等）
  // 兼容 log.api('分类', '消息', meta) 调用；此前该方法缺失，
  // 导致 export.js 中 21 处 log.api(...) 抛 "log.api is not a function"，
  // 备份/导出/恢复全部失败。
  api: (cat, msg, meta) => log('INFO', cat, msg, meta),
