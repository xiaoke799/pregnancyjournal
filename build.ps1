...
$Workspace   = "D:\pregnancy-journal"
$PkgDir      = Join-Path $Workspace "pregnancyjournal"
...
# ---------- Step 6: fnpack 打包 ----------
Step "6/7 fnpack 打包"
Get-ChildItem $Workspace -Filter "*.fpk" -File | Remove-Item -Force
Push-Location $Workspace
& ".\fnpack_tool.exe" build --directory $PkgDir
if ($LASTEXITCODE -ne 0) { Pop-Location; throw "fnpack 打包失败" }
Pop-Location

$fpk = Get-Item (Join-Path $Workspace "pregnancyjournal.fpk") -ErrorAction SilentlyContinue
